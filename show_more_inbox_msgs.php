...
}else
{
       ...
       $q="select msg_id as mid,user_id as u,seen as sn,msg as m ,msg_clr as mc,bg_color as bgc,time as tm from messages where  msg_id!=$last_id and msg_id between   $initial_msg_id and $last_id and sender_id=$user_id and delt=0  order by msg_id desc limit 25";
       ...
       if($r)
       {
	    ...
	    while($row=  mysqli_fetch_array($r,MYSQLI_ASSOC))
	    {
	      ...
	      $ppl_usrename=$rp['u'];
			    ...
			    $qw="update messages set senter_seen=0 where msg_id=$msg_id";
	           mysqli_query($dbc, $qw);
		...
		$again_seen=1;
		 ...
		 if(mysqli_num_rows($r12)>0)
		 {
              ...
              {
              echo '<div onclick="show_more_inbox_msgs('.$msg_id.')">Show More Messages</div>';
              }
       }
}