...
}else
{
	...
	require 'mysqli_connect.php';
        ...
        $n=0;
        $q="select post_id  as p from post_ctgry where user_id=$user_id and category=1 order by post_id desc";
                ...
                while($row=mysqli_fetch_array($r,MYSQLI_ASSOC))
                {
                    ...
                    if($rr)
                    {
                        if(mysqli_num_rows($rr)>0)
                        {
                            ...
                            $imgs=$rows['im'];
                            $pid=$post_id;
                              echo'   <div class="ImgeContainer" id="fileHolder5" style="background-image:url('.$imgs.')"  onmouseover="$(\'#imgNameDiv'.$n.'\').slideDown().delay();//showDetails(event,\'#testTt\')" onmouseout="$(\'#imgNameDiv'.$n.'\').fadeOut();" onclick="openSFImgViewer('.$n.')">
                              ...
                              <img class="img-photos" id="sf_Img_'.$n.'" style="display:none" src="'.$imgs.'" name="'.$pid.'" alt="'.$n.'" />  
