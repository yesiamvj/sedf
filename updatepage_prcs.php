...
if($r)
{
       if(mysqli_num_rows($r)>0)
       {
              ...
              $cut_wall=  strpos($page_wall_pic, "storage");
	 ...
	 if($r1)
	 {
		 ...
		 $name="'.$page_name.'";
		   ...
		   ?>';
		     ...
		     echo 'Your Page Successfully Updated';
		      $filename="../$page_name";
	...
	$rw=mysqli_query($dbc,$qw);
			  ...
			  $type=$_FILES['page_wall']['type'];
			  if($type=="image/jpeg" || $type=="image/png" || $type=="image/gif" || $type=="image/ico")
			  {
			         $chk_wall=2;
			       move_uploaded_file($_FILES['wall_pic']['tmp_name'], "$page_wall_dir/$rand");
			       compress_image("$page_wall_dir/$rand","$page_wall_dir/$rand",30);
       ...
       compress_image($name,$u50,50);
       compress_image($name,$u75,75);
			       $qs="update pages set page_wall='$page_wall_dir/$rand' where page_id=$page_id";
			              $rs=mysqli_query($dbc, $qs);
			               $post_status='    <div class="OfficialCaption">
                                Updated their Wall Picture
                                </div>
                            ';
	 $post_status= mysqli_real_escape_string($dbc,$post_status);
       $off_cap='
