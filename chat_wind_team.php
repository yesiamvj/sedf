  ...
  if($r)
  {
      if(mysqli_num_rows($r)>0)
      {
	    ...
	    $team2_id=$team_id;
              ...
              $qw="select members_id as m from group_members where group_id=$team_id and members_id=$user_id";
              ...
              <div class="ChatWindowHold" id="ChatWind'.$cnt.'" >
                  ...
                  <div class="ChatWindowOut">
                      ...
                      <div class="ChatWindTtl" style="background-color:'.$theme.';"  >
                          ...
                          <div class="ChWin_ChaterNme"  onclick="$(\'#ChatWind'.$cnt.'\').slideUp();$(\'#ChatIcon'.$cnt.'\').fadeIn();"><div>'.$battle_name.'</div>  <span id="for_chat_online_sts'.$my_id.'" style="display:none;"><div class="Ch_Usr_Sts" style="background-color: green" ></div></span> </div>
                          <div class="ChWin_Tools" style="background-color: '.$theme.';" >
                              ...
                              <div class="ChWin_Close" onclick="$(\'#on_live_grp_convrse'.$my_id.'\').val(\'0\');$(\'#usrcwteam'.$battle_id.'\').fadeOut();$(\'#myonusrcnt'.$cnt.'\').val(\'usrhidn'.$cnt.'\');">X<input type="hidden" id="chwnusrcnt" value="'.$cnt.'" /></div>
                          </div>
                      </div>
...
echo'<div id="forlivegrpconv'.$my_id.'"></div>';
  echo'<input type="hidden" value="'.$my_id.'" id="whoislive'.$my_id.'">
            ...
            ';
                      ...
                      $q="SELECT group_id as gp_id,member_id as u,msg as m,time as t,grp_msg_id as mid,msg_clr as clr,bg_clr as bgc,day as d,seen as sns,show_name as sh from group_messages where group_id=$team1_id or group_id=$team2_id order by grp_msg_id desc limit 10";
                      $r=mysqli_query($dbc,$q);
                      if($r)
                      {
                           $m=0;
...
';
                            }else
                            {
                              ...
                              <div class="CW_Conv_InCont" id="CW_Conv_InCont'.$my_id.'" style="background-color:'.$bgclr.'">';
                                if($show_name==1)
                              {
                                  echo'<b>'.$mem_name.'</b>';
                              }else
...
if($(\'#team_chatloded\').val()==="1")
{
       ...
       $(\'#team_chatloded\').val("0");
                        ...
                        {
                            echo'<div id="for_rmv_dh_mr_msgs'.$my_id.'" ><div style="text-align:center;cursor:pointer;" onclick="showpre_team_msgs(\''.$battle_id.'\',\'#for_rmv_dh_mr_msgs'.$my_id.'\',\'#full_conv_hold'.$my_id.'\',\''.$myteam.'\',\''.$my_id.'\',\''.$msg_id.'\')">Show Previous Messages</div></div>';
                        }
                        echo'
              </div>
