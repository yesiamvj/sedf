...
}else
{
	...
	$cnt=$_REQUEST['cnt'];
	$last_id=$_REQUEST['cmnt_id'];
	
	require 'mysqli_connect.php';
	$q="select cmnt_id as c from post_comments limit 1";
	...
	if($r)
	{
	  ...
	  $q="SELECT comment_word as cmnt ,smiley as sm,cmnt_id as cid,comment_time as time,commenter_useri_d as uid,comment_media as media ,comment_color as clr from post_comments where cmnt_id between 1 and $last_id and post_id=$post_id  order by cmnt_id desc limit 10";
...
$f1_format=substr($cmnt_media,(strlen($cmnt_media)-3),strlen($cmnt_media));
                        ...
                        echo'
                        <div id="totlprevcmnts'.$post_id.'" class="totlprevcmnts">
	                      <img src="'.$p_pic.'" class="commenterFace" style="width:100px;height:100px;"/>
                      
	              <div class="commentUserArr"></div>
                        ...
                        <div id="hisComment'.$post_id.'" class="hisComment">';
                                if(strlen($comment)>50)
                                {
                                        ...
                                        echo'<span class="commentText" style="color: '.$cmnt_clr.';">'.$comment.'</span><br/>';
             ...
             ';                  
                   ...
                   echo'</div>
                         <span id="forCommentStatus">
                                    <img class="ico_PostResp" onclick="whoAreThey(\'.postLikers\')" class="cmntstslike" id="postStatusLike" src="icons/post-like-black.png" alt="Likes"/>
                            <span onclick="whoAreThey(\'.postLikers\')"  id="post-sf-currentLike">'.$cl.'</span>
                            <img class="ico_PostResp"  id="postStatusLike"  src="icons/post-unlike-black.png" alt="Likes"/>
                            <span class="unlikeCount" id="post-sf-currentUnlike">'.$cul.'</span>
                            <img  class="ico_PostResp" src="icons/post-sf-sts-rate.png" alt="Likes"/><span onclick="whoAreThey(\'.postRatings\')" class="rateCount" id="post-sf-currentRate">'.$crt.'</span>
 ...
 <div class="commentUserArr"></div>
                ...
                {
	      echo '<div id="show_more_cmnt_btn" onclick="show_more_comments('.$cmnt_id.','.$cnt.','.$post_id.')">Show More Comments</div>';
                }else
                {
