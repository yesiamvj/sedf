...
}else
{
       ...
       $q="select msg_id as mid,sender_id as u,seen as sn,msg as m ,msg_clr as mc,bg_color as bgc,time as tm,senter_seen as sn from messages where user_id=$user_id and delt=1 order by msg_id desc limit 25";
       ...
       if($r)
       {
	    ...
	    while($row=  mysqli_fetch_array($r,MYSQLI_ASSOC))
	    {
	         ...
	         $senter_seen=$row['sn'];
			  ...
			  $qe="select med_img as m from small_pics where user_id=$senter_id";
		    ...
		    echo '   <div class="tabContTtl">
                                    Seen <div class="tabCount"> 20 Items</div>
                                </div>
                             ';    
	             }else
	             {
		     echo '   <div class="tabContTtl">
                                 Not Seen <div class="tabCount"> 20 Items</div>
                                </div>
                             ';   
	             }
		 ...
		 if(mysqli_num_rows($r12)>0)
		 {
              ...
              {
	    echo '<div id="show_more_delt" onclick="show_more_delt_msgs('.$msg_id.')">Show More Messages</div>';
              }
       }
}