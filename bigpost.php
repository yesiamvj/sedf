 ...
 else
{
    ...
    <link rel="stylesheet" href="'.$_SESSION['css'].'thePost.css"/> 
      ...
      <link rel="stylesheet" href="'.$_SESSION['css'].'theBigThate.css"/> 
   
     <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
  <script src="thePost.js" type="text/javascript"></script>';
        
        
        
        $user_id=$_SESSION['user_id'];
      $post_idn=$_REQUEST['post_id'];
       
      $cnt=$_REQUEST['tot_cont'];
        $post_idmedn=$_REQUEST['med_n'];
        ...
        function allpost($post_idmedn,$cap_text_clr,$cap_back_clr,$shared_uid,$commented_uid,$rated_uid,$uliked_uid,$liked_uid,$shared_fname,$commenter_fname,$uliked_fnam,$rated_fname,$liked_fname,$rate_cnt,$pers_unlike,$pers_like,$tot_unlikes,$tot_likes,$btm_meme,$top_meme,$thum_nails,$tot_files,$post_pdfs,$post_audios,$post_videos,$post_files,$shared_name,$shared_user_name,$shared_time,$post_images,$pvs,$post_loc,$post_feel,$post_feelwhile,$officiale,$with_ppl_user_name,$with_ppl_name,$poster_user_name,$p_pic,$post_id,$poster_id,$post_caption,$post_time,$poster_name,$imgc,$cnt,$lc,$ulc,$ratc,$cmcnt,$dncnt,$shrcnt,$mylike,$myunlike,$myrate,$likeres,$unlikeres,$dwnres,$cmntres,$shareres,$rateres,$ctgry,$discr,$shared_with_userid,$shared_to_me,$c_name,$shared_userid,$tot_num_post, $next_postid,$prev_postid,$page_name,$my_category,$page_id,$tot_cnt)    
        {
	           ...
	           <input type="hidden" value="'.$next_postid.'" id="nextPostNumber'.$post_id.'" />
                        </div>
...
$clean_html = $purifier->purify($dirty_html);
	 ...
	 echo ' </div>';
	        ...
	        echo '
                        ...
                        <div class="ThePostMediaHolder" id="post_media'.$post_id.'" onclick="$(\'#audi_tools'.$post_id.'\').fadeIn();$(\'#theaterActTools'.$post_id.'\').fadeIn();" >';
	         $tot_med_cont=count($post_images)+count($post_videos)+count($post_audios);
	           $tot_post_cont=$tot_med_cont+count($post_pdfs)+count($post_files);   
	        if(count($post_images)>0)
	        {
	               echo ''.$top_meme.'';
	              echo ' <img src="'.$post_images[$post_idmedn].'" onclick="//showBigScreen(\''.$post_id.'\',\''.count($post_images).'\',\'1\',\''.$next_postid.'\')" class="img_ThePost" /><input type="hidden" id="totalMedItms_'.$post_id.'" value="'.  count($post_images).'" /><input type="hidden" value="'.$post_idmedn.'" id="thisPostMedCount'.$post_id.'"/>';
	               echo ''.$btm_meme.'';
	        }
	        if(count($post_videos)>0)
	        {
		     ...
		     $thum_nail="";       
	               ...
	               echo ' <video src="'.$post_videos[1].'" preload="none" onclick="//showBigScreen(\''.$post_id.'\',\''.count($post_images).'\',\'1\',\''.$next_postid.'\')" class="img_ThePost" controls poster="'.$thum_nails[0].'"/></video><input type="hidden" id="totalMedItms_'.$post_id.'" value="'.  count($post_images).'" /><input type="hidden" value="'.$post_idmedn.'" id="thisPostMedCount'.$post_id.'"/>';
	              
	        }
	         if(count($post_audios)>0)
	               {
		echo '<audio src="'.$post_audios[1].'" class="img_ThePost"  preload="none" controls/></audio>';     
	               }
	             if($post_idmedn>$tot_med_cont || $tot_med_cont==0)
	               {
		     if(count(($post_pdfs)>0) || count($post_files)>0)
		     {
		            for($t=1;$t<=count($post_pdfs);$t++)
		            {
			  $download=$post_pdfs[$t];
			    $name=strpos($post_pdfs[$t],"postpdfs/")+15;
			     $pdf_name=substr($post_pdfs[$t],$name,  strlen($post_pdfs[$t]));
			     $file="PDF";
			      ...
			      echo ' <div class="ThePostPdfFileNameHolder">
                                ...
                                <div class="TP_PDF_Dets">
                                    <div class="TP_PDF_Name">
                                     '.htmlentities($pdf_name).'  
                                    </div>
                                    <div class="TP_PDF_size">
                                       <a href="'.htmlentities($post_pdfs[$t]).'" target="_blank"><div class="TP_PDF_Preview">Preview</div></a> <a href="'.htmlentities($download).'"><div class="TP_PDF_download">Download</div></a>
                                    </div>
                                </div>
	  ...
	  if($likeres==1)
	         ...
	         echo '<div class="TPSH_Itm">';
                        if($mylike==1)
                        {
                            ...
                            echo '<img class="ico_PostResp" onclick="Likedone(1,\'#like'.$cnt.'\','.$post_id.','.$cnt.');incread_count(\'.like_cnt'.$post_id.'\',1)"  id="like'.$cnt.'" src="../web/icons/post-sf-liked.png" alt="like"/> ';
                           
                        }else
                        {
                        echo '<img class="ico_PostResp" onclick="Likedone(1,\'#like'.$cnt.'\','.$post_id.','.$cnt.');incread_count(\'.like_cnt'.$post_id.'\',1)"  id="like'.$cnt.'" src="../web/icons/post-sf-like.png" alt="like"/> ';
	       ...
	       echo '<div class="TPSH_Itm">';
                        ...
                        if($myunlike==1)
                        {
                            ...
                            echo' <img class="ico_PostResp" onclick="Likedone(0,\'#unlike'.$cnt.'\','.$post_id.','.$cnt.');incread_count(\'.unlike_cnt'.$post_id.'\',0)"  id="unlike'.$cnt.'" src="../web/icons/post-sf-unliked.png" alt="Unlike"/>';
                            
                        }else
                        {
                            
                            echo' <img class="ico_PostResp" onclick="Likedone(0,\'#unlike'.$cnt.'\','.$post_id.','.$cnt.');incread_count(\'.unlike_cnt'.$post_id.'\',0)"  id="unlike'.$cnt.'" src="../web/icons/post-sf-unlike.png" alt="Unlike"/>
                            ';
                        }
                        }
	       echo '</div>';
	 ...
	 echo '    <div class="TPSH_Itm" onclick="ViewComment(\'#commentContentFull_gen'.$cnt.'\','.$tot_cnt.','.$post_id.')" >
	 ...
	 echo '         
                            <div class="TPSH_Itm" onclick="ViewComment(\'#commentContentFull_gen'.$cnt.'\','.$tot_cnt.','.$post_id.')" >
                                <img src="../web/icons/postSts/comments_2.png" />
                            </div>';      
	       }
	       if($officiale==0)
...
{
       $(\'.sf-commentInput\').keyup(function(event)
       {
             ...
             $(\'#submit_post_cmnt'.$cnt.'\').click();
	                 ...
	                 <input type="hidden" id="tot_cmnt_smiley'.$tot_cnt.'" value="1"/>
                        <div id="commentContentFull_gen'.$cnt.'" class="commentContentFull">
	              <div class="newCommentHolder" >
                            <span class="commentTitle" id="commentTitle'.$tot_cnt.'" title="Click to view All Comments" onclick="viewprecmnt(\'#previousComments'.$tot_cnt.'\','.$post_id.','.$tot_cnt.')">Comments &nbsp;&nbsp;&nbsp;<img style="width: 10px;height:10px;" src="../web/icons/expnd-dwn.png" alt="down"/></span><span id="refresh_cmnt'.$cnt.'" style="cursor:pointer;" onclick="refresh_cmnt('.$post_id.',\'#previousComments'.$tot_cnt.'\',\''.$tot_cnt.'\')">New</span>
		   ...
		   <div class="for_cmnt_smiley" style="display:none;" id="for_show_smiley'.$tot_cnt.'"></div>
                                   ...
                                   <div class="forCmntArrow" id="forCmntArrow"></div>
                                    ...
                                    <input class="submitCmnt" id="submit_post_cmnt'.$cnt.'" type="button" onclick="uploadcomment('.$post_id.',0,0,'.$tot_cnt.')" value="Comment"/>
                                    </div>
...
<input type="hidden" value="1" id="wch_clkd"/>
                            ...
                            <div class="TPRD_Itm" id="Post_Likers_PID" name="'.$post_id.'" title="'.$cnt.'" onclick="showPostLikers('.$cnt.')"  >
                                ...
                                <div class="TPRD_Itm"  id="Post_Raters_PID" title="'.$cnt.'" name="'.$post_id.'" onclick="showPostRaters('.$cnt.')">
                                    ...
                                    <div class="TPRD_RespCount" >
                                        <img class="TPRD_RatedIcon" src="../web/icons/post-rated-'.$myrate.'.png" />
                                    </div>
                                    <div class="TPRD_RespItmName">
                                        Rating
                                    </div>
                                ...
                                <div class="TPRD_Itm" title="'.$cnt.'" id="Post_Sharers_PID" name="'.$post_id.'"  onclick="showPostSharers('.$cnt.')">
                                    ...
                                    <div class="TPRD_RespCount" id="ThePostShares" >
                                        <span class="share_cnt'.$cnt.'">'.count($shared_fname).'</span>
                                    </div>
                                    <div class="TPRD_RespItmName">
                                        Shares
                                    </div> 
	      ...
	      <div id="audi_tools'.$post_id.'" style="//display:none;">     
                    <div class="theaterPostDrivers" id="theaterActTools'.$post_id.'"  >
                        ...
                        <div class="theaterPrev" onclick="showNextBigScreen(\''.$post_id.'\',\''.count($post_images).'\',\''.$post_idmedn.'\',\''.$next_postid.'\','.$tot_num_post.')">
                           Next
                        </div>
                        <div class="theaterNext" onclick="showPrevBigScreen(\''.$post_id.'\',\''.count($post_images).'\',\''.$post_idmedn.'\',\''.$prev_postid.'\','.$tot_num_post.')">
                           Prev
                        </div>
                    ...
                    <div class="theaterPostDriverSafetyLayer" id="theatToolSafetyLayer"  style="display: none;" onmouseover="hideTheaterTools()" ></div>
                    <div class="rightReadyMouseChecker" onmouseover="makeTheaterToolsVisible()" ></div>
...
$ratc=0;
	     ...
	     $seen_cnt=0;
             ...
             }else
             {
                    ...
                    $aps="SELECT post_id as pid,user_id as uid,allpeople as ap,allrelation as alr,share as shr,page_id as p,officiale as of_cl,me as ni from post_permision  where post_id=$post_idn limit 1";
			    ...
			    $tm_ct=0;
			     ...
			     if(strpos($post_audio,"post/audio")>0)
			     {
		           ...
		           $iam_seen_post=0;
                                        ...
                                        {
		             $iam_seen_post=1;
                                        allpost($post_idmedn,$cap_text_clr,$cap_back_clr,$shared_uid,$commented_uid,$rated_uid,$uliked_uid,$liked_uid,$shared_fname,$commenter_fname,$uliked_fnam,$rated_fname,$liked_fname,$rate_cnt,$pers_unlike,$pers_like,$tot_unlikes,$tot_likes,$btm_meme,$top_meme,$thum_nails,$tot_files,$post_pdfs,$post_audios,$post_videos,$post_files,$shared_name,$shared_user_name,$shared_time,$post_images,$pvs,$post_loc,$post_feel,$post_feelwhile,$officiale,$with_ppl_user_name,$with_ppl_name,$poster_user_name,$p_pic,$post_id,$poster_id,$post_caption,$post_time,$poster_name,$imgc,$cnt,$lc,$ulc,$ratc,$cmcnt,$dncnt,$shrcnt,$mylike,$myunlike,$myrate,$likeres,$unlikeres,$dwnres,$cmntres,$shareres,$rateres,$ctgry,$discr,$shared_with_userid,$shared_to_me,$c_name,$shared_userid,$tot_num_post, $next_postid,$prev_postid,$page_name,$my_category,$page_id,$tot_cnt);
                                        }else
                                         {
